...
<div>
    ...
    <div class="col-12 float-left padding-0-5-10">
        ...
        <div class="col-6 style-text-align-right font-size-12">
            <span @click="chooseSpecitalHotel(dataHotel)">Xem tất cả</span></div>
    </div>
    <div class="col-12">
        <carousel :centerMode="false" :perPage="1" :navigationEnabled="false" paginationColor="#7e7e7e" paginationPosition="bottom" :perPageCustom="[[480, 1], [768, 2], [1024, 2]]">
            <slide v-for="(dataHotel, index) in data" :key="index">
                <div class="col-11 style-box-shadow">
                    ...
                    <div class="col-8 style-padd">
                        <p class="style-bold font-size-title">{{ dataHotel.name }}</p>
                        <p class="padding-p"> <a @click="chooseSpecitalHotel(dataHotel)" class="style-back-cam font-size-title"><i class="fas fa-star"></i> <span> {{ dataHotel.averageMark }} ({{ dataHotel.totalFavorite }}) </span></a></p>
                        ...
                        <div class="float-right style-padd-10">
                            <p><span class="style-qua-dem font-size-title"> Qua đêm </span><span class="font-size-title"> VND</span><span class="style-gia font-size-title-price">{{ dataHotel.lowestPriceOvernight }}</span><span class="style-gia-giam font-size-title">{{ dataHotel.lowestOneDay }}</span></p>
                        </div>
                    </div>
...
export default {
    ...
    methods: {
        chooseSpecitalHotel(dataHotel) {
            ...
            this.$router.push({ name: 'Category', params: { Sn: 2 }})
        }
    },
    // computed: {
    //     roomData() {
